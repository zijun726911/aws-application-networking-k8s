...
func (r *TLSRouteRule) Matches() []RouteMatch {
	...
	/*
		#TODO there is no Matches field in TLSRoute
		for _, routeMatch := range r.r.Matches {
			routeMatches = append(routeMatches, &TLSRouteMatch{routeMatch})
		}
	*/
	return routeMatches
}
...
type TLSRouteMatch struct {
	/* # TODO
	no TLSRouteMatch
	m gwv1alpha2.TLSRouteMatch
	*/
	//m gwv1beta1.HTTPRouteMatch
}

func (m *TLSRouteMatch) Headers() []HeaderMatch {
	var headerMatches []HeaderMatch
	/* TODO
	no header match for TLSRoute
	for _, headerMatch := range m.m.Headers {
		headerMatches = append(headerMatches, &TLSHeaderMatch{headerMatch})
	}
	*/
	return headerMatches
}

func (m *TLSRouteMatch) Method() *gwv1beta1.HTTPMethod {
	// TODO no *gwv1alpha2.TLSMethodMatch {
	return nil
}

func (m *TLSRouteMatch) Equals(routeMatch RouteMatch) bool {
	other, ok := routeMatch.(*TLSRouteMatch)
	if !ok {
		return false
	}
...
type TLSHeaderMatch struct {
	/* TODO, no header match
	m gwv1alpha2.TLSHeaderMatch
	*/
	m gwv1beta1.HTTPHeaderMatch
}

func (m *TLSHeaderMatch) Type() *gwv1beta1.HeaderMatchType {
	// TODO return m.m.Type
	return m.m.Type
}

func (m *TLSHeaderMatch) Name() string {
	return string(m.m.Name)
}

func (m *TLSHeaderMatch) Value() string {
	return m.m.Value
}

func (m *TLSHeaderMatch) Equals(headerMatch HeaderMatch) bool {
	other, ok := headerMatch.(*TLSHeaderMatch)
	if !ok {
		return false
	}
